...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name_err) && empty($description_err) && empty($price_err) && empty($stock_err) && empty($category_err) && empty($image_err)) {
        ...
        $sql = "INSERT INTO products (name, description, price, stock_quantity, category_id, image, status) VALUES (?, ?, ?, ?, ?, ?, 'active')";

        if ($stmt = mysqli_prepare($conn, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssdiss", $param_name, $param_description, $param_price, $param_stock, $param_category, $param_image);
